...
public class RaidZoneManager
{
    ...
    private static boolean checkClanAreaKickTask(Player activeChar, Integer numberBox)
    {
        ...
        if (clan != null)
        {
            ...
            {
                if (!player.isInsideZone(ZoneId.CUSTOMBOSS) || player.getClan() == null)
                    continue;
    ...
    private static boolean checkAllyAreaKickTask(Player activeChar, Integer numberBox, Collection<Player> world)
    {
        ...
        if (activeChar.getAllyId() != 0)
        {
            ...
            {
                if (!player.isInsideZone(ZoneId.CUSTOMBOSS) || player.getAllyId() == 0)
                    continue;
    ...
    private static boolean checkClanAreaKickTaskB(Player activeChar, Integer numberBox)
    {
        Map<String, List<Player>> zergMap = new HashMap<>();
        Clan clan = activeChar.getClan();
        ...
        if (clan != null)
        {
            for (Player player : clan.getOnlineMembers())
            {
                if (!player.isInsideZone(ZoneId.BOSS) || player.getClan() == null)
                    continue;
                String zerg1 = activeChar.getClan().getName();
                String zerg2 = player.getClan().getName();
    ...
    private static boolean checkAllyAreaKickTaskB(Player activeChar, Integer numberBox, Collection<Player> world)
    {
        Map<String, List<Player>> zergMap = new HashMap<>();
        ...
        if (activeChar.getAllyId() != 0)
        {
            for (Player player : world)
            {
                if (!player.isInsideZone(ZoneId.BOSS) || player.getAllyId() == 0)
                    continue;
                String zerg1 = activeChar.getClan().getAllyName();
                String zerg2 = player.getClan().getAllyName();
    ...
    public boolean checkClanAreaB(Player activeChar, Integer numberBox, Boolean forcedTeleport)
    {
        if (checkClanAreaKickTaskB(activeChar, numberBox))
        {
            if (forcedTeleport)
            {
                activeChar.sendMessage("Allowed only " + numberBox + " clan members on this area!");
    ...
    public boolean checkAllyAreaB(Player activeChar, Integer numberBox, Collection<Player> world, Boolean forcedTeleport)
    {
        if (checkAllyAreaKickTaskB(activeChar, numberBox, world))
        {
            if (forcedTeleport)
            {
                activeChar.sendMessage("Allowed only " + numberBox + " ally members on this area!");
