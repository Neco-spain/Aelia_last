...
public final class Formulas
{
	...
	public static double calcBlowDamage(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss)
	{
		...
		//damage *= ClassBalanceManagerCustom.getInstance().getBalancedClass(AttackType.Blow, attacker, target);
	...
	public static final double calcPhysDam(Creature attacker, Creature target, L2Skill skill, byte shld, boolean crit, boolean ss)
	{
		...
		if (crit){
				if (skill != null && skill.getSkillType() == L2SkillType.PDAM)
				{
					...
					//damage *= ClassBalanceManagerCustom.getInstance().getBalancedClass(AttackType.PhysicalSkillCritical, attacker, target);
					int keyId = (target instanceof Player) ? target.getActingPlayer().getClassId().getId() : (target instanceof Monster) ? -1 : -2;
					...
					//damage *= ClassBalanceManagerCustom.getInstance().getBalancedClass(AttackType.Crit, attacker, target);
				}
		}
		else{
				if (skill != null && skill.getSkillType() == L2SkillType.PDAM){
					damage *= ClassBalanceManager.getInstance().getBalancedClass(AttackType.PhysicalSkillDamage, attacker, target);
					//damage *= ClassBalanceManagerCustom.getInstance().getBalancedClass(AttackType.PhysicalSkillDamage, attacker, target);
				}
				else{
					damage *= ClassBalanceManager.getInstance().getBalancedClass(AttackType.Normal, attacker, target);
					//damage *= ClassBalanceManagerCustom.getInstance().getBalancedClass(AttackType.Normal, attacker, target);
					}
		}
	...
	public static final double calcMagicDam(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss, boolean bss, boolean mcrit)
	{
		...
		if (Config.MAGIC_FAILURES && !calcMagicSuccess(attacker, target, skill))
		{
			...
			damage *= CustomConfig.MCRIT; //4 , 22/11 change from 2.5
		...
		if (mcrit)
		{
			...
			//damage *= ClassBalanceManagerCustom.getInstance().getBalancedClass(AttackType.MCrit, attacker, target);
			int keyId = (target instanceof Player) ? target.getActingPlayer().getClassId().getId() : (target instanceof Monster) ? -1 : -2;
			...
			//damage *= ClassBalanceManagerCustom.getInstance().getBalancedClass(AttackType.Magic, attacker, target);
		}
	 ...
	 * @return true in case of critical hit
	 */
	//custom for fix crit rate
	public static final boolean calcCritBow(double rate,boolean moreCrits)
	{
		 //custom Crit rate fixes, default return rate > Rnd.get(1000);
		...
		if(rate < 400) {
			_rate = Rnd.get(CustomConfig.calcCrirMod400);
			if(Config.CUSTOM_DEVELOPER)
				System.out.println("Player crit: " + rate + " Random Rate: " + _rate +  " Crit Chance: " + (df2.format( (rate/_rate) * 100) ) + "%");
			return rate >  _rate;
		}
		else if(rate < 300) {
			_rate = Rnd.get(CustomConfig.calcCrirMod300);
			if(Config.CUSTOM_DEVELOPER)
				System.out.println("Player crit: " + rate + " Random Rate: " + _rate +  " Crit Chance: " + (df2.format( (rate/_rate) * 100) ) + "%");
			return rate > _rate;
		}

		_rate = Rnd.get(CustomConfig.calcCrirMod500);
		if(Config.CUSTOM_DEVELOPER)
			System.out.println("Player crit: " + rate + " Random Rate: " + _rate +  " Crit Chance: " + (df2.format( (rate/_rate) * 100) ) + "%");
		return rate > _rate;
	}
	...
	public static final void calcLethalHit(Creature attacker, Creature target, L2Skill skill)
	{
		...
		{
			//custom no lethal to RB's,Mobs
			if(target instanceof RaidBoss || target instanceof Monster)
				return;
			...
			else if (target instanceof Player) // If is a active player set his HP and CP to 1
			{
				...
				if (!player.isInvul())
				{
					...
					{
						//custom lethal dmg
						/*double lethalDmg = (player.getMaxHp() / 2 ) - Rnd.get(50,100); // skill.getPower() ;
						if(player.getCurrentHp() < lethalDmg ) {
							player.setCurrentHp(1);
							if(Config.CUSTOM_DEVELOPER)
								System.out.println("Lethal Set current Hp to 1");
						}
						else
							player.setCurrentHp( Math.max(player.getCurrentHp() - lethalDmg , 1) ); //1

						if(Config.CUSTOM_DEVELOPER)
							System.out.println("Lethal Dmg: " + lethalDmg);
						*/
						player.setCurrentHp(1);
	...
	private static double getCustomModifier(Creature attacker)
	{
		final double Modifier = attacker.calcStat(Stats.SKILL_CHANCE, 1.0, null, null);
...
//	public static boolean calcEffectSuccess(Creature attacker, Creature target, EffectTemplate effect, L2Skill skill, byte shld, boolean bss)
...

	{
		...
		final double customModifier =  getCustomModifier(attacker);//attacker.calcStat(Stats.SKILL_CHANCE, 1, null, null);

	 	int keyId = (target instanceof Player) ? target.getActingPlayer().getClassId().getId() : (target instanceof Monster) ? -1 : -2;
		final double balancer = SkillBalanceManager.getInstance().getSkillValue(skill.getId() + ";" + keyId, SkillChangeType.Chance, target);

		double rate = Math.max(1, Math.min((baseChance * statModifier * skillModifier * mAtkModifier * lvlModifier * customModifier * balancer), 99));
		//double rate = Math.max(1, Math.min(rat * customModifier,99));

		if (Config.DEVELOPER) {
			LOGGER.info("calcEffectSuccess(): name:{} eff.type:{} power:{} statMod:{} skillMod:{} mAtkMod:{} lvlMod:{} total:{}%  customMod:{} Balancer:{}.", skill.getName(), type.toString(), baseChance, String.format("%1.2f", statModifier), String.format("%1.2f", skillModifier), String.format("%1.2f", mAtkModifier), String.format("%1.2f", lvlModifier), String.format("%1.2f", rate), customModifier, String.format("%1.2f",balancer));
		}
	...
	public static boolean calcSkillSuccess(Creature attacker, Creature target, L2Skill skill, byte shld, boolean bss)
	{
		...
		final double lvlModifier = getLevelModifier(attacker, target, skill);
		final double customModifier =  getCustomModifier(attacker);//attacker.calcStat(Stats.SKILL_CHANCE, 1, null, null);
//		final double rate = Math.max(1, Math.min((baseChance * statModifier * skillModifier * mAtkModifier * lvlModifier * customModifier), 99));
		...
		if (Config.DEVELOPER) {
			LOGGER.info("calcSkillSuccess(): name:{} type:{} power:{} statMod:{} skillMod:{} mAtkMod:{} lvlMod:{} total:{}% customMod:{} Balancer:{}.", skill.getName(), skill.getSkillType().toString(), baseChance, String.format("%1.2f", statModifier), String.format("%1.2f", skillModifier), String.format("%1.2f", mAtkModifier), String.format("%1.2f", lvlModifier), String.format("%1.2f", rate), customModifier, String.format("%1.2f",balancer));
		}
		return (Rnd.get(100) < rate);
	}
