...
public class MapRegionData implements IXmlReader
{
	...
	public Location getLocationToTeleport(Creature creature, TeleportType teleportType)
	{
		...
		if (teleportType == TeleportType.PVP_ZONE) {
		...
		return !player.isInsideZone(ZoneId.CHANGE_PVP_ZONE) ? getClosestTown(player).getRandomLoc() : getTown(9).getRandomLoc();
	}

	public final void respawnCharacter(Player character){
		if (character == null || !character.isDead() || !(character instanceof Player)){
			System.out.println("There was a problem on respawn character insinde PVP zone (MapRegionData#respawnCharacter");
			return;
		}
		character.doRevive();
		...
		if(RandomZoneManager.getInstance().getCurrentZone() != null && character.isInsideZone(ZoneId.CHANGE_PVP_ZONE))
			character.teleportTo(RandomZoneManager.getInstance().getCurrentZone().getLoc(), 30);
		else
			character.teleportTo(82635, 148798, -3464, 30);
	}
	...
	private final TownZone getClosestTown(Creature creature)
	{
		...
		//custom 
		return getTown(16); // Default to floran(16) , custom changed to 9 Giran .
	}
