...
public final class RequestRestartPoint extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		//custom events
        if ((CTFEvent.isStarted() && CTFEvent.isPlayerParticipant(player.getObjectId())) || (DMEvent.isStarted() && DMEvent.isPlayerParticipant(player.getObjectId())) /*|| (LMEvent.isStarted() && LMEvent.isPlayerParticipant(activeChar.getObjectId()))*/ || (TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(player.getObjectId()))) {
            return;
        }
	...
	private void portPlayer(Player player)
	{
		...
		else if (_requestType == 4)
		{
			...
			if (player.isGM())
				loc = player.getPosition();
			else if(!player.isGM() && RandomZoneManager.getInstance().getCurrentZone() != null){
				MapRegionData.getInstance().getLocationToTeleport(player, TeleportType.PVP_ZONE);
