...
//	INSERT INTO `clan_privs_test` (`clan_id`,`rank`,`privs`) VALUES (1,1,1) ON DUPLICATE KEY UPDATE `privs` = VALUES(`privs`)
	...
	{
		//custom allow clan skills with 0 points
	...
	public SubPledge createSubPledge(Player player, int type, int leaderId, String subPledgeName)
	{
		...
		{
			if (pledgeType < SUBUNIT_KNIGHT1) // royal , custom default 5000
				takeReputationScore(5000);
			else if (pledgeType > SUBUNIT_ROYAL2) // knight
				takeReputationScore(10000); //custom default 10000
		}
	...
	public boolean levelUpClan(Player player)
	{
		...
		switch (_level)
		{
			...
			case 5:
				if (_reputationScore >= 10000 && getMembersCount() >= CustomConfig.CLAN_MEMBERS_LVL6) //custom , default: 30
				{
			...
			case 6:
				if (_reputationScore >= 20000 && getMembersCount() >= CustomConfig.CLAN_MEMBERS_LVL7) //custom , default: 80
				{
			...
			case 7:
				if (_reputationScore >= 40000 && getMembersCount() >= CustomConfig.CLAN_MEMBERS_LVL8)//custom , default: 120
				{
