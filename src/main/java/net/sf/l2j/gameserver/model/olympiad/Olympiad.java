...
public class Olympiad
{
	...
	private static final String GET_EACH_CLASS_LEADER_CURRENT = "SELECT characters.char_name, olympiad_nobles.olympiad_points from olympiad_nobles, characters WHERE characters.obj_Id = olympiad_nobles.char_id AND olympiad_nobles.class_id = ? AND olympiad_nobles.competitions_done >= " + CustomConfig.ALT_OLY_MIN_MATCHES + " AND characters.accesslevel = 0 ORDER BY olympiad_nobles.olympiad_points DESC, olympiad_nobles.competitions_done DESC, olympiad_nobles.competitions_won DESC LIMIT 10";
	
	private static final String TEST = "SELECT characters.char_name, olympiad_nobles.class_id from olympiad_nobles, characters WHERE characters.obj_Id = olympiad_nobles.char_id AND olympiad_nobles.class_id = ?"; // AND characters.accesslevel = 0
	
	private static final String TEST_UPDATE = "UPDATE olympiad_nobles SET class_id = ?  WHERE char_id = ?";
	...
	private void load()
	{
		...
		//Customs
		OlyAnnounce.olyperiod(_period,_olympiadEnd);
	...
	*/
	...
	protected void setNewOlympiadEnd()
	{
		...
		currentTime.add(Calendar.WEEK_OF_YEAR, CustomConfig.OlympiadEvery);
		currentTime.set(Calendar.DAY_OF_WEEK, CustomConfig.OlympiadDay);
		
		currentTime.set(Calendar.AM_PM, Calendar.AM);
		currentTime.set(Calendar.HOUR_OF_DAY, CustomConfig.OlympiadHour);

		
		currentTime.set(Calendar.MINUTE, CustomConfig.OlympiadMinute);
		currentTime.set(Calendar.SECOND, 0);
		_olympiadEnd = currentTime.getTimeInMillis();
	 ...
	 */
	public synchronized void saveNobleData()   //custom protected
	{
...
//			PreparedStatement ps = con.prepareStatement(GET_EACH_CLASS_LEADER))
			//custom
			PreparedStatement ps = con.prepareStatement(GET_EACH_CLASS_LEADER_CURRENT))
...
//					names.add(rs.getString(CHAR_NAME));
					names.add(rs.getString(CHAR_NAME)+  "                      " + rs.getInt(POINTS));
	...
	public Map<String,Integer> getClassNames(int classId)
	{
		...
		
		{
			...
			{
				while (rs.next())
					names.put(rs.getString(CHAR_NAME),rs.getInt(CLASS_ID));
		...
		{
			LOGGER.error("Couldn't getClassNames", e);
		}
		return names;
	}
	
	public Integer getClassNames1(int classId) {
		if(_nobles.isEmpty())
			return null;
		
		try {
			for (Map.Entry<Integer, StatsSet> noble : _nobles.entrySet()) {
				if (noble.getValue().getInteger(CLASS_ID) == classId) {
					return noble.getValue().getInteger(CLASS_ID);
				}
			}
		}
		catch (Exception e)
		{
			LOGGER.error("Couldn't getClassNames1", e);
		}
		
		return null;
	}
	
	public void updateDonate1(int classId,int newClassId) {
		if(_nobles.isEmpty())
			return ;
		...
		try {
			for (Map.Entry<Integer, StatsSet> noble : _nobles.entrySet()) {
				if (noble.getValue().getInteger(CLASS_ID) == classId) {
					System.out.println("Found " + classId + " charId and replacing...");
					noble.getValue().replace(Olympiad.CLASS_ID, newClassId);
				}
			}
		}
		catch (Exception e)
		{
			LOGGER.error("Couldn't updateDonate1", e);
		}
		
		saveNobleData();
	}
	
	public synchronized void updateDonate(int classId,int charId)   //custom protected
	{
		if (_nobles == null || _nobles.isEmpty())
			return;
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			for (Map.Entry<Integer, StatsSet> noble : _nobles.entrySet())
			{
				...
				if (set == null)
					continue;
				if(noble.getKey() != charId)
					continue;

				System.out.println("Found " + charId + " charId and replacing...");
					ps = con.prepareStatement(TEST_UPDATE);
					ps.setInt(1, classId);
					ps.setInt(2, charId);
					noble.getValue().replace(Olympiad.CLASS_ID, classId);
		...
		{
			LOGGER.error("Couldn't update Donate change class.", e);
		}
	}
	...
	//Customs
	public long getMillisToOlympiadEndv()
	{
		return (_olympiadEnd - Calendar.getInstance().getTimeInMillis());
	}

	public long getMillisToValidationEndv()
	{
		if (_validationEnd > Calendar.getInstance().getTimeInMillis())
			return (_validationEnd - Calendar.getInstance().getTimeInMillis());
	...
	public long getMillisToWeekChangev()
	{
		if (_nextWeeklyChange > Calendar.getInstance().getTimeInMillis())
			return (_nextWeeklyChange - Calendar.getInstance().getTimeInMillis());
	...
	public OlympiadState getPeriod(){
		return _period;
	}
	
	
	//custom  ,olympiad points show on ranking
	public int getPoints(String objId)
	{
		int result = 0;
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			final PreparedStatement statement = con.prepareStatement("SELECT olympiad_points FROM olympiad_nobles WHERE char_id = ?");
		...
		{
			LOGGER.info(Level.WARNING, "getPoints error(Olympiad, line 800):Could not load olympiad points:", e );
		}
		return result;
	}
