...
public abstract class AbstractOlympiadGame
{
	...
	protected static SystemMessage checkDefection(Player player)
	{
		...
		//custom events
        if (CTFEvent.isPlayerParticipant(player.getObjectId()) || DMEvent.isPlayerParticipant(player.getObjectId()) /*|| LMEvent.isPlayerParticipant(player.getObjectId()) */ || TvTEvent.isPlayerParticipant(player.getObjectId()) || player.isInTournament()) {
	...
	protected static final void removals(Player player, boolean removeParty)
	{
		...
		// Remove Buffs
		player.stopAllEffectsExceptThoseThatLastThroughDeathOly();
		...
		else
		{
			...
			{
				summon.stopAllEffectsExceptThoseThatLastThroughDeathOly();
				summon.abortAttack();
				summon.abortCast();
			}
		}
	...
	protected static final void buffPlayer(Player player)
	{
		...
		//custom Give acumen to mage classes
		if (!player.isMageClass())
		{
			skill = SkillTable.getInstance().getInfo(1086, 1); // Haste 1
			if (skill != null)
	...
	protected static final void portPlayerBack(Player player)
	{
		...
		player.getSavedLocation().clean();//this must be here or will not work!

		for(L2Skill sk : player.getSkills().values()){
			if(sk.getId() >= 10050 && sk.getId() <= 10119){
				player.removeSkill(sk.getId(), false);
