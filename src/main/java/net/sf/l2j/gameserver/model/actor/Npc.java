...
import net.sf.l2j.gameserver.model.actor.instance.BufferNew;
...
import Customs.mods.fakepc.FakePc;
...
public class Npc extends Creature
{
	...
	public void onActionShift(Player player)
	{
		...
		if (player.isGM())
			sendNpcInfos(player);
		//custom Rb shift click
		else if(CustomConfig.ALT_GAME_VIEWNPC && (getTemplate().isType("RaidBoss") || getNpcId() == CustomConfig.monsterId )) //rb and party farm mobs
		{
			NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
			html.setFile("data/html/mods/mobinfo/mobinfo.htm");
			...
			//html.replace("%aggro%", (this instanceof Attackable) ? ((Attackable) this).getAggroRange() : 0);
	...
	public void onDecay()
	{
		...
		if (_spawn != null)
			//custom corpse life fix
//			_spawn.doRespawn();
			_spawn.doCustomRespawn();
...
//			onCustomDecay();
	...
	//custom remove final for augment manager npc
	public void showChatWindow(Player player, String filename)
	{
		player.incInteracts();
//		System.out.println("Interacts1: " + player.getInteracts());
		
		if(this instanceof Buffer || this instanceof BufferNew){
			if (player.isInCombat() || player.getPvpFlag() != 0){
				player.sendMessage("Can't use this action while in combat/flag.");
				player.sendPacket(ActionFailed.STATIC_PACKET);
				return;
			}
		}
