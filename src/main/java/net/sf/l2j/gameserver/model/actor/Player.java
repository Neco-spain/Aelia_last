...
import Customs.data.DressMeData;
...
import dev.l2j.tesla.autobots.ui.AdminActions;
...
public class Player extends Playable
{
	...
	protected Player(int objectId, PlayerTemplate template, String accountName, Appearance app)
	{
		...
		//custom
	        this._originalTitleColorVip = 0;
	        this._originalNameColorVip = 0;
	        this._trySkinId = 0;
	...
	public static Player create(int objectId, PlayerTemplate template, String accountName, String name, byte hairStyle, byte hairColor, byte face, Sex sex)
	{
		...
		player._originalTitleColorVip = 0;
		player._originalNameColorVip = 0;
        player._trySkinId = 0;
	...
	public final static PlayerTemplate getBaseTemplateEvents()
	{
		return PlayerData.getInstance().getTemplate(ClassId.FORTUNE_SEEKER);
	}
	...
	public void updatePvPFlag(int value)
	{
		...
		//custom , not check if player flag , only on kill(playable)
//		PvpColor.checkColorPlayer(this);

		if (value != 0 && isInFunEvent() )
		{
			setPvpFlag(0);
	...
	public void useEquippableItem(ItemInstance item, boolean abortAttack)
	{
		...
		{
			//custom
			if (item.isArmor() && item.getArmorItem().getItemType() == ArmorType.HEAVY &&
				(  getClassId().getId() == ClassId.HAWKEYE.getId() || getClassId().getId() == ClassId.SAGGITARIUS.getId()
				|| getClassId().getId() == ClassId.GHOST_SENTINEL.getId()  ||  getClassId().getId() == ClassId.PHANTOM_RANGER.getId()
				|| getClassId().getId() == ClassId.MOONLIGHT_SENTINEL.getId() ||  getClassId().getId() == ClassId.SILVER_RANGER.getId()
				|| getClassId().getId() == ClassId.WIND_RIDER.getId() || getClassId().getId() == ClassId.PLAINS_WALKER.getId()
				|| getClassId().getId() == ClassId.ADVENTURER.getId() ||  getClassId().getId() == ClassId.TREASURE_HUNTER.getId()
				|| getClassId().getId() == ClassId.GHOST_HUNTER.getId() || getClassId().getId() == ClassId.ABYSS_WALKER.getId() ) )

			{
				sendMessage("Your class cannot equip heavy armor.");
				return;
			}
	...
	public boolean reduceAdena(String process, int count, WorldObject reference, boolean sendMessage)
	{
		...
		if (count > 0)
		{
			...
			//custom auto pot
			SoulShots.handleTeleportPotion(this, 728);
			SoulShots.handleTeleportPotion(this, 1539);
			SoulShots.handleTeleportPotion(this, 5592);
	...
	//custom respawn protection
	private ScheduledFuture<?> _protectTaskRevive;
	public void setReviveProtection(boolean protect)
	{
		if (protect)
		{
			if (_protectTaskRevive == null)
				_protectTaskRevive = ThreadPool.schedule(() ->
				{
					setReviveProtection(false);
					sendMessage("The spawn protection has ended.");
				}, 3 * 1000);
		}
		else
		{
			_protectTaskRevive.cancel(true);
			_protectTaskRevive = null;
		}
		broadcastUserInfo();
	}
	public boolean isReviveProtected()
	{
		return _protectTaskRevive != null;
	}
	...
	{
		if (_clan == null || _clan.getAllyId() == 0)
			return 0;
	...
	public void sendPacket(L2GameServerPacket packet)
	{
		if (_client != null) {
			...
			if(packet instanceof NpcHtmlMessage) {
				incInteracts();
//				System.out.println("Interacts: " + getInteracts());
			}
	...
	//custom dressme
	public Armor getActiveArmorItem()
	{
		final ItemInstance armor = getInventory().getPaperdollItem(Inventory.PAPERDOLL_CHEST);
		return (armor == null) ? null : (Armor)  armor.getItem();
	}

	public Armor getActiveLegItem()
	{
		final ItemInstance armor = getInventory().getPaperdollItem(Inventory.PAPERDOLL_LEGS);
		return (armor == null) ? null : (Armor)  armor.getItem();
	}


	public Item getActiveShield()
	{
		final ItemInstance shield = getInventory().getPaperdollItem(Inventory.PAPERDOLL_LHAND);

		return (shield == null) ? null :  shield.getItem();
	}
	...
	public boolean doDie(Creature killer)
	{
		...
		//custom on player die , set karm to 0
		if (getKarma() > 0)
			setKarma(0);
	...
	private void onDieDropItem(Creature killer)
	{
		...
		if ((!isInsideZone(ZoneId.PVP) || pk == null) && (!isGM() || Config.KARMA_DROP_GM))
		{
			...
			{
				if(isVip()) {
					dropPercent = Config.KARMA_RATE_DROPVIP;
					dropEquip = Config.KARMA_RATE_DROP_EQUIPVIP;
					dropEquipWeapon = Config.KARMA_RATE_DROP_EQUIP_WEAPONVIP;
					dropItem = Config.KARMA_RATE_DROP_ITEMVIP;
					dropLimit = Config.KARMA_DROP_LIMITVIP;
				}
				else {
...
//						itemDropPercent = itemDrop.getItem().getType2() == Item.TYPE2_WEAPON ? dropEquipWeapon : dropEquip;
						itemDropPercent = itemDrop.getItem().getType2() == Item.TYPE2_WEAPON ? (itemDrop.isAugmented() ? dropEquip : dropEquipWeapon) : dropEquip;
	...
	public void onKillUpdatePvPKarma(Playable target)
	{
		...
		//custom anti farm
		if(!(this instanceof Autobot))
			if(!antiFarm.checkAntiFarm(player ,targetPlayer))
				return;
		...
		if (checkIfPvP(target) || (targetPlayer.getClan() != null && getClan() != null && getClan().isAtWarWith(targetPlayer.getClanId()) && targetPlayer.getClan().isAtWarWith(getClanId()) && targetPlayer.getPledgeType() != Clan.SUBUNIT_ACADEMY && getPledgeType() != Clan.SUBUNIT_ACADEMY) || (targetPlayer.getKarma() > 0 && Config.KARMA_AWARD_PK_KILL))
		{
			if (target instanceof Player)
			{
				...
				//custom PVP announce
				if (CustomConfig.ANNOUNCE_KILL)
					World.toAllOnlinePlayers(SystemMessage.getSystemMessage(SystemMessageId.S1_S2).addString(CustomConfig.ANNOUNCE_PVP_MSG.replace("$killer", getName()).replace("$target", targetPlayer.getName())).addZoneName(targetPlayer.getPosition()));


				//custom  give reward to support classes
				giveSupportRewards(player);

				if(CustomConfig.REWARD_PVP_KILL && (isInsideZone(ZoneId.CHANGE_PVP_ZONE) || isInsideZone(ZoneId.PARTY) || isInsideZone(ZoneId.CUSTOMBOSS)) ) {
					if(isVip())
						addItem("PvP", CustomConfig.PVP_REWARD_ID, (CustomConfig.PVP_REWARD_AMMOUNT * 2), this ,true); //Custom VIP x2 pvp reward
					else
						addItem("PvP", CustomConfig.PVP_REWARD_ID, CustomConfig.PVP_REWARD_AMMOUNT, this ,true);
				}
		...
		else if (targetPlayer.getKarma() == 0 && targetPlayer.getPvpFlag() == 0)
		{
			...
			//custom PK announce
			if (CustomConfig.ANNOUNCE_KILL)
				World.toAllOnlinePlayers(SystemMessage.getSystemMessage(SystemMessageId.S1_S2).addString(CustomConfig.ANNOUNCE_PK_MSG.replace("$killer", getName()).replace("$target", targetPlayer.getName())).addZoneName(targetPlayer.getPosition()));
		...
		public void giveSupportRewards(Player player){
			if(player.getParty() != null){//if player is in party and is support class give reward .
				for(Player member : player.getParty().getMembers()){
					if(member == this)
						continue;
					if(member.getPvpFlag() != 0 && isSupportClass(member.getActiveClass(), true)
							&& Util.checkIfInRange(2000, player, member, true) && Rnd.get(11) == 0 ){
						member.setPvpKills(member.getPvpKills() + 1);
						member.sendPacket(new ExShowScreenMessage("You got a PvP kill from your party members,enjoy!", 4000, SMPOS.BOTTOM_RIGHT, true));
		...
		public boolean isSupportClass(int _class, boolean includeMidDDs) { //static
			if (includeMidDDs) {
				switch (_class) {
					case 16:
					case 17:
	...
	public boolean isFarmer(ClassId id){
		if(id == ClassId.MYSTIC_MUSE || id == ClassId.STORM_SCREAMER  || id == ClassId.SOULTAKER || id == ClassId.ARCHMAGE || id == ClassId.DUELIST || id == ClassId.DREADNOUGHT)
			return true;
		return false;
	}
	...
	{
		return super.isInvul() || isSpawnProtected() || isReviveProtected();
	}
			...
			PreparedStatement ps = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			try (ResultSet rs = ps.executeQuery())
			{
				while (rs.next())
				{
					...
					//custom donate shop
					player.setDonateColor(rs.getString("Donatecolor"));
					player.setVipNColor(rs.getString("VipNcolor"));
					player.setVipTColor(rs.getString("VipTcolor"));

					player.setOriginalNColor(rs.getString("OriginalNColor"));
					player.setOriginalTColor(rs.getString("OriginalTColor"));


					//Custom Hardcoded for custom maps helpers... (Solo dungeon maps)
					int databaseX = rs.getInt("x");
					int databaseY = rs.getInt("y");
					int databaseZ = rs.getInt("z");

					//System.out.println(databaseX+ " "+databaseZ);
					if(databaseX < -65000 && databaseZ > 5000) {
						System.out.println("Known bug core1. Character: "+ rs.getString("char_name"));
						databaseX = 82635;
						databaseY = 148798;
						databaseZ = -3464;
					}
					...
					//startup
					player.setFirstLog(rs.getInt("first_log"));

					player.setSelectArmor(rs.getInt("select_armor"));
					player.setSelectWeapon(rs.getInt("select_weapon"));
					player.setSelectClasse(rs.getInt("select_classe"));

					player.setBaseClassDona(rs.getInt("beforeDonaChangeClass"));
	...
	public void storeCharBase()
	{
			...
			PreparedStatement ps = con.prepareStatement(UPDATE_CHARACTER))
		{
			...
			ps.setString(47, getDonateColor());
			ps.setString(48, getVipNColor());
			ps.setString(49, getVipTColor());
			ps.setString(50, getOriginalNColor());
			ps.setString(51, getOriginalTColor());
				...
				try (ResultSet rs = ps.executeQuery())
				{
					while (rs.next())
					{
						...
						if (skill.hasEffects())
						{
							...
							{
								final L2Effect ef = et.getEffect(this, this, skill);
								if (ef != null)
								{
	...
	public boolean isAutoAttackable(Creature attacker)
	{
		...
		//custom events
//		if(attacker instanceof Player && this.isInFunEvent() && ((Player) attacker).isInFunEvent())
//			return true;
		...
		if (attacker instanceof Playable) {
			...
			//custom events
			if (attacker instanceof Player && cha.isInFunEvent() && isInFunEvent())
				return true;
			...
			if (getClan() != null) {
				final Siege siege = CastleManager.getInstance().getActiveSiege(this);
				if (siege != null) {
					if (CustomConfig.ENABLE_SIEGE_HIT && cha.isInSiege() && isInSiege() && cha.isInsideZone(ZoneId.SIEGE) && isInsideZone(ZoneId.SIEGE))
						return true;
				...
				//custom When you request a clan to war and they have not accept it yet, they are able to attack you w/o ctrlPressed and take you PK by auto attacks.
				if (getClan().isAtWar() && getClan().isAtWarWith(cha.getClanId()) && cha.getClan().isAtWar() && cha.getClan().isAtWarWith(getClanId())
						&& !wantsPeace() && !cha.wantsPeace() && !isAcademyMember())
					return true;
			}
	...
	private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		//custom not attackable npc
		if (CustomConfig.DISABLE_ATTACK_NPC_TYPE && target instanceof Npc && !(target instanceof SiegeFlag))
		{
			String type = ((Npc) target).getTemplate().getType();
			if ((!CustomConfig.LIST_ALLOWED_NPC_TYPES.contains(type)) && (skill.isOffensive()))
			{
		...
		if (skill.isOffensive())
		{
			...
			if (!target.isAttackable() && !getAccessLevel().allowPeaceAttack())
			{
				...
				sendMessage("You cannot hit that NPC.");
				sendPacket(ActionFailed.STATIC_PACKET);
				return false;
			}
	...
	public boolean checkPvpSkill(WorldObject target, L2Skill skill)
	{
		...
		//custom tour
		if(target instanceof Player && isInTournament() && ((Player) target).isInTournament())
			return true;


		//custom hit enemy wihout CTRL
		if(target instanceof Player && ((Player) target).isInFunEvent())
			return true;

		if(target instanceof Player && getClan() == ((Player) target).getClan() && isInsideZone(ZoneId.CHANGE_PVP_ZONE) && ((Player) target).isInsideZone(ZoneId.CHANGE_PVP_ZONE))
			return true;
		...
		if (skill.isDebuff() || skill.isOffensive())
		{
			...
			//custom hit team player
			if(target instanceof Player && isInFunEventRan() && ((Player) target).isInFunEventRan() && loadCustomMods.areTeamMates(this, (Player) target))
				return false;

			// Duel
			//custom duel fix , before duel starts player could do debuff skills like surrender etc.. && getDuelState() == DuelState.DUELLING
			if (isInDuel() && targetPlayer.isInDuel() && getDuelId() == targetPlayer.getDuelId() && getDuelState() == DuelState.DUELLING)
				return true;
			...
			if (aClan != null && tClan != null)
			{
				if (aClan.isAtWarWith(tClan.getClanId()) && tClan.isAtWarWith(aClan.getClanId())) {
					...
					if (skill.isOffensive() && getClan().isAtWar() && getClan().isAtWarWith(targetPlayer.getClanId()) && targetPlayer.getClan().isAtWar() && targetPlayer.getClan().isAtWarWith(getClanId()))
						return true;

					if (aClan.isAtWar() &&  tClan.isAtWar() &&!wantsPeace() && !targetPlayer.wantsPeace() && !isAcademyMember())
						return true;
			...
			if (targetPlayer.getPvpFlag() == 0 && targetPlayer.getKarma() == 0)
			{
				...
				if (isCtrlPressed && skill.getTargetType() == SkillTargetType.TARGET_AREA)
						return true;

				//custom can use Chargedam,AOE skills if not flag(force bluster etc.) skill.getSkillType().equals(L2SkillType.CHARGEDAM)
			//	if ( (skill.getEffectRange() > 0 && isCtrlPressed && getTarget() == target && skill.isDamage() )
			//			|| ( (skill.getSkillType().equals(L2SkillType.CHARGEDAM) || skill.isAOE()) && isCtrlPressed ) ) {
				if ( skill.getEffectRange() > 0 && isCtrlPressed && getTarget() == target
						&& (skill.isDamage() || skill.getTargetType() == SkillTargetType.TARGET_AREA || skill.getSkillType() == L2SkillType.CHARGEDAM ) ){
					return true;
				}
				return false;
			}
	...
	{
		if (isInTournament() && !TournamentConfig.REMOVE_BUFFS_ON_DIE)
			return;
	...
	public final void stopAllEffectsExceptThoseThatLastThroughDeathOly()
	{
		super.stopAllEffectsExceptThoseThatLastThroughDeathOly();
		updateAndBroadcastStatus(2);
	}
	/**
	...
	{
		if (hero) // Allow skills to all classes - removed -> && _baseClass == _activeClass
		{
	...
	boolean _isCustomHero;

	public boolean isCustomHero()
	{
		return _isCustomHero;
	}

	public void setCustomHero(boolean hero)
	{
		_isCustomHero = hero;
		setHero(hero);
	}
	...
	public boolean addSubClass(int classId, int classIndex)
	{
		...
		try
		{
			if (_subClasses.size() == CustomConfig.ALLOWED_SUBCLASS || classIndex == 0 || _subClasses.containsKey(classIndex))
				return false;
	...
	public void onPlayerEnter()
	{
		...
		VoteDao.cangetVoteBuff(this);
//			VoteManagerTask.getInstance().add(this);
	...
	public final void onTeleported()
	{
		...
		if (Config.PLAYER_SPAWN_PROTECTION > 0 && !isReviveProtected())
	...
	private synchronized void cleanup()
	{
		try
		{
			...
			if(VoteManagerTask.getInstance().contain(this))
				VoteManagerTask.getInstance().remove(this);
		...
		//	LMEvent.onLogout(this);
			...
			{
				setInstance(InstanceManager.getInstance().getInstance(0), true);
				teleportTo(82635, 148798, -3464, 25);
			}
			...
			if(this instanceof Autobot)
				AdminActions.INSTANCE.despawnAutobotCustom((Autobot)this);
	...
	public void calculateDeathPenaltyBuffLevel(Creature killer)
	{
		...
		//custom  , events
	    if (loadCustomMods.isPlayerInEvent(this) && loadCustomMods.isEventStarted())
	        return;
	...
	public boolean canAttackCharacter(Creature cha)
	{
		...
		if (cha instanceof Playable)
		{
			...
			//custom attack in event without control
			if(isInFunEvent())
				return true;
	...
	public boolean checkSummonTargetStatus(WorldObject target)
	{
		...
		if (!CTFEvent.onEscapeUse(target.getObjectId()) || !DMEvent.onEscapeUse(target.getObjectId()) || /*!LMEvent.onEscapeUse(target.getObjectId()) ||*/ !TvTEvent.onEscapeUse(target.getObjectId()))
		{
			sendPacket(SystemMessageId.YOUR_TARGET_IS_IN_AN_AREA_WHICH_BLOCKS_SUMMONING);
			return false;
		}
	...
	//custom fix event dward height and radius
	public double getCollisionRadiusEvents()
	{
		return getBaseTemplateEvents().getCollisionRadiusBySex(getAppearance().getSex());
	}

	//custom fix event dward height and radius
	public double getCollisionHeightEvents()
	{
		return getBaseTemplateEvents().getCollisionHeightBySex(getAppearance().getSex());
	}
	...
	{
		return isRegisteredInTournament() && _tournament.getState() == TournamentState.STARTED;
	}
    ...
    private long _vip_endTime;
	protected int _originalTitleColorVip = 0;
	protected int _originalNameColorVip = 0;

	public void setTitleColorVip(int value) {
		_originalTitleColorVip = value;
	}
	public void setNameColorVip(int value) {
		_originalNameColorVip = value;
	}

	public int getTitleColorVip() {
		return  _originalTitleColorVip;
	}

	public int getNameColorVip() {
		return _originalNameColorVip;
	}
    ...
    public void setVip(final boolean val) {
        this._isVip = val;
    }

    public long getVipEndTime() {
        return this._vip_endTime;
    }

    public void setVipEndTime(final long val) {
        this._vip_endTime = val;
    }

    public void rewardVipSkills() {
        for (final Integer skillid : CustomConfig.VIP_SKILLS.keySet()) {
            final int skilllvl = CustomConfig.VIP_SKILLS.get(skillid);
    ...
    public void setEndTime(final String process, final int val) {
        final Calendar calendar = Calendar.getInstance();
        calendar.add(5, val);
        final long end_day = calendar.getTimeInMillis();
        if (process.equals("vip")) {
            this._vip_endTime = end_day;
        }
    }


    private int _trySkinId;
    public int getTrySkin() {
        return _trySkinId;
    }

    public void setTrySkin(final int b) {
        _trySkinId = b;
    }
	...
	int _teamEvent;
	public void setTeamEvent(int team)
	{
		_teamEvent = team;
	}
	...
	{
		return (  (TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(getObjectId())) || (CTFEvent.isStarted() && CTFEvent.isPlayerParticipant(getObjectId()))
			|| (DMEvent.isStarted() && DMEvent.isPlayerParticipant(getObjectId()))  );
	}

	public boolean isInFunEventRan()
	{
		return ( (TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(getObjectId())) || (CTFEvent.isStarted() && CTFEvent.isPlayerParticipant(getObjectId()) )   );
	}

	public boolean isInFunEventRand()
	{
		return ( (TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(getObjectId())) || (CTFEvent.isStarted() && CTFEvent.isPlayerParticipant(getObjectId()) ));
	}
	...
	public int _originalNameInt;
	public int _originalNameCTF;
	...
	private int sameLoc = 0;

	public void increaseSameLoc() {
		++sameLoc;
	}
	public void setSameLoc(int x) {
		sameLoc = x;
	}
	public int getSameLoc() {
		return sameLoc;
	}
	...
	public int _lastXEvent;
	public int _lastYEvent;
	public int _lastZEvent;

	public int getLastXEvent()
	{
		return _lastXEvent;
	}

	public int getLastYEvent()
	{
		return _lastYEvent;
	}

	public int getLastZEvent()
	{
		return _lastZEvent;
	}

	public void setLastCordsEvent(int x, int y, int z)
	{
		_lastXEvent = x;
		_lastYEvent = y;
		_lastZEvent = z;
	...
	private int sameLocEvent = 0;

	public void increaseSameLocEvent() {
		++sameLocEvent;
	}
	public void setSameLocEvent(int x) {
		sameLocEvent = x;
	}
	public int getSameLocEvent() {
		return sameLocEvent;
	}
	...
	//startup
	  ...
	  {
	    return this._select_armor;
	  }

	  public boolean getSelectWeapon()
	  {
	    return this._select_weapon;
	  }

	  public boolean getSelectClasse()
	  {
	    return this._select_classe;
	  }

	  public boolean getFirstLog()
	  {
	    return this._first_log;
	  }
	  ...
	  {
	    this._select_armor = false;
	    if (selectArmor == 1) {
	      this._select_armor = true;
	    }
	  }

	  public void setSelectWeapon(int selectWeapon)
	  {
	    this._select_weapon = false;
	    if (selectWeapon == 1) {
	      this._select_weapon = true;
	    }
	  }

	  public void setSelectClasse(int selectClasse)
	  {
	    this._select_classe = false;
	    if (selectClasse == 1) {
	      this._select_classe = true;
	    }
	  }

	  public void setFirstLog(boolean firstlog)
	  {
	    this._first_log = firstlog;
	  }

	  public void setSelectArmor(boolean selectArmor)
	  {
	    this._select_armor = selectArmor;
	  }

	  public void setSelectWeapon(boolean selectWeapon)
	  {
	    this._select_weapon = selectWeapon;
	  }

	  public void setSelectClasse(boolean selectClasse)
	  {
	    this._select_classe = selectClasse;
	  }

	  public void updateDonaClass()
	  {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE characters SET beforeDonaChangeClass=? WHERE obj_id=?");

			int _class;
			if(getBaseClassDona() > 0)
				_class = getBaseClassDona();
			else
				_class = 0;

			statement.setInt(1, _class);
	  ...
	  public void updateSelectArmor()
	  {
	    ...
	    {
	      LOGGER.info("could not set char select armor:" + e);
	    }
	  }
	  ...
	  public void updateSelectWeapon()
	  {
	    ...
	    {
	      LOGGER.info("could not set char select weapon:" + e);
	    }
	  }
	  ...
	  public void updateSelectClasse()
	  {
		...
		public void setCancelTask(boolean value)
		{
			isCancelTask = value;
		}
		...
		public void setAutoPot(int id, Future<?> task, boolean add)
		{
		 ...
		 //custom  ,donate shp
		private String donateColor = "";

		//custom  ,donate shop
		public String getDonateColor()
		{
			return donateColor;
		}

		public String setDonateColor(String colors)
		{
			return donateColor = colors;
		}

		private String vipNColor = CustomConfig.VIP_NCOLOR;

		//custom  ,donate shop
		public String getVipNColor()
		{
			return vipNColor;
		}

		public String setVipNColor(String colors)
		{
			return vipNColor = colors;
		}


		private String vipTColor = CustomConfig.VIP_TCOLOR;
		...
		public String setVipTColor(String colors)
		{
			return vipTColor = colors;
		}


		private boolean haveDonateColor = false;

		public boolean gethaveDonateColor() {
			return haveDonateColor;
		}
		public void sethaveDonateColor(boolean x) {
			haveDonateColor = x;
		}


		private String originalNColor = "FFFF77";

		//custom  ,donate shop
		public String getOriginalNColor()
		{
			return originalNColor;
		}

		public String setOriginalNColor(String colors)
		{
			return originalNColor = colors;
		}


		private String originalTColor = "FFFF77";

		//custom  ,donate shop
		public String getOriginalTColor()
		{
			return originalTColor;
		}

		public String setOriginalTColor(String colors)
		{
			return originalTColor = colors;
		}
		...
		//custom vote
		 ...
		 private boolean _isVoting;
		     ...
		     private Map<VoteSite, Long> _lastVoteTimestamps = new HashMap<VoteSite, Long>() {{
		         ...
		         put(VoteSite.TOPCO, -1L);
		         put(VoteSite.L2VOTE, -1L);
		         put(VoteSite.L2JBRASIL, -1L);
		     }};
	    ...
	    public boolean hwidVotesContain(String hwid) {
	 	...
	 	{
	 		return PlayerData.getInstance().getTemplate(_baseClass).getClassName();
	 	}

	 	private int baseClassBeforeChangeClass = 0;
	 	public void setBaseClassDona(int classId) {
	 		baseClassBeforeChangeClass = classId;
	 	}
	 	public int getBaseClassDona() {
	 		return baseClassBeforeChangeClass;
	 	}

	 	public final String getClassNameOnChangedClass() {
	 		if(baseClassBeforeChangeClass != 0)
	 			return PlayerData.getInstance().getTemplate(baseClassBeforeChangeClass).getClassName();
		...
		private boolean isPlayerAfk = false;

		public void setPlayerAfk(boolean is) {
			isPlayerAfk = is;
		}
		...
		private int interactWithNpc = 0;

		public void setInteracts(int x) {
			interactWithNpc = x;
		}
		public void incInteracts() {
			++interactWithNpc;
		}

		public int getInteracts() {
		...
		private boolean autoSS = true;

		public void setAutoPot(boolean x) {
			autoPot = x;
		}
		public boolean getAutoPot() {
			return autoPot;
		}

		public void setautoSS(boolean x) {
			autoSS = x;
		}
		public boolean getautoSS() {
			return autoSS;
		}
	...
	private int visual_hair = 0;
	private int visual_chest = 0;

	private int visual_legs = 0;
	private int visual_gloves = 0;
	private int visual_boots = 0;

	public void setVisual(int id, int hair, int chest, int legs, int gloves, int boots){
		visual = id;
		visual_hair = hair;
		visual_chest = chest;
		visual_legs = legs;
		visual_gloves = gloves;
		visual_boots = boots;
		broadcastUserInfo();
	}
	...
	public int getVisualChest()
	{
		return visual_chest;
	}
	public int getVisualLegs() { return visual_legs; }
	public int getVisualGloves()
	{
		return visual_gloves;
	}
	public int getVisualBoots()
	{
		return visual_boots;
	}
	...
	public void getAchievemntData()
	{
		...
		{
			System.out.println("[ACHIEVEMENTS ENGINE GETDATA]" + e);
		}
	}
	...
	public void saveAchievementData(int achievementID, int objid)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			if (achievementID == 7 || achievementID == 8 || achievementID == 12 )//|| achievementID == 11 || achievementID == 13
			{
		...
		{
			System.out.println("[ACHIEVEMENTS SAVE GETDATA]" + e);
		}
	}
