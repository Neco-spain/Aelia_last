...
public class BossZone extends ZoneType
{
	...
	protected void onEnter(Creature character)
	{
		...
		if (character instanceof Player)
		{
			...
			if (player.isGM() || _invadeTime == 0 || CustomConfig.ENTER_ZONE_BOSS)
				return;
	...
	{
		return RaidZoneManager.getInstance().checkClanAreaB(activeChar, _maxClanMembers, true);
	}

	public boolean MaxAllyMembersOnArea(Player activeChar)
	{
		return RaidZoneManager.getInstance().checkAllyAreaB(activeChar, _maxAllyMembers, World.getInstance().getPlayers(), true);
	}
	...
	protected void onExit(Creature character)
	{
		...
		if (character instanceof Playable)
		{
			if (character instanceof Player)
			{
				...
				if (CustomConfig.FLAG_RB || player.getPvpFlag() > 0) {
